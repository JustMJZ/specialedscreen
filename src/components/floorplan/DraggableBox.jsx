...
const DraggableBox = ({ box, onUpdate, isEditMode, containerRef, onEdit, students }) => {
  ...
  const assignedStudents = (box.assignedStudents || [])
  ...
  useEffect(() => {
    ...
    const handleMove = (x, y) => {
      ...
      if (isDragging)
        onUpdate({
          ...box,
          left: Math.max(5, Math.min(x - rect.left - dragOffset.x, rect.width - box.width - 5)),
          top: Math.max(5, Math.min(y - rect.top - dragOffset.y, rect.height - box.height - 5)),
        });
      if (isResizing)
        onUpdate({
          ...box,
          width: Math.max(30, Math.min(x - rect.left - box.left, rect.width - box.left - 5)),
          height: Math.max(30, Math.min(y - rect.top - box.top, rect.height - box.top - 5)),
        });
    };
  ...
  return (
    <div
      ref={ref}
      className={`absolute rounded flex flex-col items-center justify-center shadow-lg select-none overflow-hidden ${isEditMode ? 'cursor-move' : ''}`}
      style={{
        top: box.top,
        left: box.left,
        width: box.width,
        ...
        zIndex: isDragging || isResizing ? 100 : 4,
      }}
      onMouseDown={(e) => {
        if (!isEditMode || e.target.dataset.resize || e.target.dataset.edit) return;
        e.preventDefault();
    ...
    >
      ...
      <div
        className="text-white font-bold truncate px-1 text-center w-full"
        style={{ fontSize: Math.max(10, 18 * scale) }}
      >
        {box.label}
      </div>
          ...
          {assignedStudents.map((s) => (
            <div key={s.id} className="flex flex-col items-center" style={{ gap: 1 }}>
              {s.photo ? (
                ...
                <div
                  className="rounded-full flex items-center justify-center border-2 border-white/60"
                  style={{
                    width: avatarSize,
                    height: avatarSize,
                    fontSize: avatarSize * 0.75,
                    backgroundColor: colors[s.name.charCodeAt(0) % 5],
                  }}
                >
                  {s.emoji}
                </div>
                ...
                <div
                  className="rounded-full flex items-center justify-center font-bold border-2 border-gray-700/30"
                  style={{
                    width: avatarSize,
                    height: avatarSize,
              ...
              <span
                ...
                style={{
                  fontSize: Math.max(6, avatarSize * 0.35),
                  lineHeight: 1,
                  color: '#1F2937'
                }}
              >
      ...
      {assignedStudents.length > 0 && (
        <button
          ...
          onClick={(e) => {
            e.stopPropagation();
            onUpdate({ ...box, hideStudents: !box.hideStudents });
          }}
      ...
      {isEditMode && (
        <>
          <button
            ...
            className="absolute top-0.5 left-0.5 w-4 h-4 bg-blue-500 rounded-full text-white text-xs flex items-center justify-center shadow"
            style={{ zIndex: 10 }}
          >
            ✏️
          </button>
