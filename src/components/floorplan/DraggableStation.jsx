...
const DraggableStation = ({
  ...
  const station = stationColors[color] ||
    COLORS.stations[color] || { bg: '#9CA3AF', light: '#E5E7EB' };
  ...
  useEffect(() => {
    ...
    const handleMove = (x, y) => {
      ...
      if (isDragging)
        update(color, {
          ...cfg,
          left: Math.max(5, Math.min(x - rect.left - dragOffset.x, rect.width - cfg.width - 5)),
          top: Math.max(5, Math.min(y - rect.top - dragOffset.y, rect.height - cfg.height - 5)),
        });
      if (isResizing)
        update(color, {
          ...cfg,
          width: Math.max(50, Math.min(x - rect.left - cfg.left, rect.width - cfg.left - 5)),
          height: Math.max(40, Math.min(y - rect.top - cfg.top, rect.height - cfg.top - 5)),
        });
    };
  ...
  return (
    <div
      ref={ref}
      className={`absolute rounded-lg shadow-md select-none overflow-hidden ${isEditMode ? 'cursor-move' : ''}`}
      style={{
        top: config.top,
        left: config.left,
        width: config.width,
        ...
        boxShadow: isTarget
          ? `0 0 12px ${station.bg}50`
          : isDropTarget || isKeyboardTarget
            ? `0 0 20px ${isKeyboardTarget ? '#3B82F6' : station.bg}80, inset 0 0 20px ${isKeyboardTarget ? '#3B82F6' : station.bg}30`
            : '0 2px 6px rgba(0,0,0,0.1)',
        zIndex: isDragging || isResizing ? 100 : 5,
        transition: 'box-shadow 0.2s ease',
      }}
      onMouseDown={(e) => {
        if (!isEditMode || e.target.dataset.resize) return;
        e.preventDefault();
    ...
    >
      <div className="flex items-center overflow-hidden" style={{ padding, gap: padding }}>
        <div
          ...
          style={{ backgroundColor: station.bg, width: dotSize, height: dotSize }}
        />
        <span
          className="font-bold text-gray-700 truncate"
          style={{ fontSize: teacherFontSize, lineHeight: 1.1 }}
        >
          {teacherName}
        </span>
      </div>
      ...
      {isEditMode && (
        <>
          <div
            data-resize="true"
            className="absolute bottom-0 right-0 cursor-se-resize flex items-end justify-end"
            style={{ width: 16, height: 16, zIndex: 10 }}
            onMouseDown={(e) => {
              e.preventDefault();
              e.stopPropagation();
              setIsResizing(true);
          ...
          {onRemove && (
            <button
              ...
              className="absolute -top-1.5 -right-1.5 w-4 h-4 bg-red-500 rounded-full text-white text-xs flex items-center justify-center shadow hover:bg-red-600"
              style={{ fontSize: 9, lineHeight: 1, zIndex: 10 }}
            >
              ✕
            </button>
          )}
          <div
            data-resize="true"
            className="absolute bottom-0 left-0 flex items-center gap-0.5"
          ...
          >
            <button
              data-resize="true"
              onClick={() => {
                const s = Math.max(0.3, Math.round(((config.avatarScale || 1.0) - 0.1) * 10) / 10);
                onUpdate(color, { ...config, avatarScale: s });
              }}
            ...
            <span className="text-gray-600 font-mono" style={{ fontSize: 8, lineHeight: 1 }}>
              {(config.avatarScale || 1.0).toFixed(1)}x
            </span>
            <button
              data-resize="true"
              onClick={() => {
                const s = Math.min(2.0, Math.round(((config.avatarScale || 1.0) + 0.1) * 10) / 10);
                onUpdate(color, { ...config, avatarScale: s });
              }}
              className="flex items-center justify-center rounded font-bold"
              style={{
