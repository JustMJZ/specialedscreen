...
const DraggableStation = ({ color, config, onUpdate, isEditMode, isTarget, containerRef, students, teacherName, stationColors, onRemove }) => {
  ...
  useEffect(() => {
    ...
    const handleMove = (x, y) => {
      ...
      if (isDragging) update(color, { ...cfg, left: Math.max(5, Math.min(x - rect.left - dragOffset.x, rect.width - cfg.width - 5)), top: Math.max(5, Math.min(y - rect.top - dragOffset.y, rect.height - cfg.height - 5)) });
      if (isResizing) update(color, { ...cfg, width: Math.max(50, Math.min(x - rect.left - cfg.left, rect.width - cfg.left - 5)), height: Math.max(40, Math.min(y - rect.top - cfg.top, rect.height - cfg.top - 5)) });
    };
