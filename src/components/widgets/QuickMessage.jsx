...
const QuickMessage = ({ message, onEdit, fontSize = 16, onFontSizeChange }) => {
  ...
  return (
    <>
      <div onClick={() => { setVal(message); setTempFontSize(fontSize); setShowModal(true); }} className="rounded-lg p-2 shadow-md cursor-pointer hover:shadow-lg transition-shadow h-full">
      ...
      {showModal && ReactDOM.createPortal(
        <div className="fixed inset-0 flex items-center justify-center z-[9999] p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="mb-4">
              ...
              <div className="flex flex-wrap gap-1.5">
                {presets.map(p => (
                  <button key={p} onClick={() => setVal(p)} className="px-2 py-1 bg-gray-100 rounded text-xs hover:bg-gray-200 transition-colors">{p}</button>
                ))}
              </div>
            </div>
            <div className="flex items-center gap-2 mb-4">
              <span className="text-sm text-gray-600 font-medium">Font Size:</span>
              <button onClick={() => setTempFontSize(Math.max(10, tempFontSize - 2))} className={`${getIconButtonClass('sm', 'secondary')} font-bold`}>A−</button>
              <span className="text-sm px-2 text-gray-700 font-bold min-w-[50px] text-center">{tempFontSize}px</span>
              <button onClick={() => setTempFontSize(Math.min(48, tempFontSize + 2))} className={`${getIconButtonClass('sm', 'secondary')} font-bold`}>A+</button>
            </div>
