...
const VoiceLevel = ({ level, onChange }) => {
  ...
  return (
    <div className="rounded-lg p-2 shadow-md h-full flex flex-col overflow-hidden bg-transparent">
      ...
      <div
        className="flex-1 flex flex-col items-center justify-center rounded-xl mb-1.5 p-1"
      ...
      >
        ...
        <div
          className="font-black uppercase tracking-wider mt-0.5"
          style={{
          ...
          {[1, 2, 3].map((bar) => (
            <div
              ...
              style={{
                ...
                animation:
                  bar <= current.bars
                    ? `bar-pulse ${0.4 + bar * 0.15}s ease-in-out infinite`
                    : 'none',
              }}
            />
          ))}
        </div>
        <div className="text-xs mt-0.5" style={{ color: current.color, opacity: 0.7 }}>
          {current.desc}
        </div>
      </div>
