        ...
        <div className="grid grid-cols-1 gap-1.5 h-full min-h-0 overflow-y-auto auto-rows-min relative">
          {ordered.map((color) => {
            ...
            return (
              <div
                key={color}
                className="flex items-center gap-2 px-2.5 py-1.5 rounded-xl relative overflow-hidden"
                style={{
                  ...
                  animation:
                    performanceMode || !inRotation
                      ? 'none'
                      : isTarget
                        ? 'group-glow 1.4s ease-in-out infinite'
                        : 'card-float 4.2s ease-in-out infinite',
                  filter: !inRotation ? 'saturate(0.4)' : 'none',
                }}
              >
                <div
                  className="absolute inset-y-0 left-0 w-1.5"
                  style={{ backgroundColor: inRotation ? bg : '#9CA3AF' }}
                />
                <div
                  ...
                  style={{
                    ...
                    animation:
                      performanceMode || !inRotation ? 'none' : 'shimmer-sweep 6s linear infinite',
                    mixBlendMode: 'screen',
                  }}
                />
                <div
                  className="w-7 h-7 rounded-full flex-shrink-0 grid place-items-center text-sm shadow-sm font-bold"
                  style={{ backgroundColor: bg, color: '#1F2937' }}
                  title={name}
                >
                ...
                <span className="text-gray-500 truncate flex-1 text-sm">
                  {grp.length > 0
                    ? `: ${grp.map((st) => st.name.split(' ')[0]).join(', ')}`
                    : ': —'}
                </span>
                <span
                ...
                {isTarget && (
                  <span
                    ...
                    style={{
                      color: bg,
                      animation: performanceMode ? 'none' : 'dot-pulse 1.1s ease-in-out infinite',
                    }}
                  >
                    ●
                  </span>
