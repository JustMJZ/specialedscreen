...
const EmptyState = ({
  ...
  return (
    ...
    >
      <div className={`${sizeClasses.icon} mb-3 opacity-40`}>{icon}</div>
      <div className={`${sizeClasses.title} font-semibold text-gray-700 mb-1`}>{title}</div>
      {description && (
        <div className={`${sizeClasses.description} text-gray-500 max-w-xs`}>{description}</div>
      )}
