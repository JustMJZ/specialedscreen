        ...
        >
          ...
          <div className="flex gap-1 mb-2">
            <button onClick={() => setColorTab('bg')}
              className={`flex-1 text-[11px] py-1 rounded font-bold ${colorTab === 'bg' ? 'bg-gray-700 text-white' : 'bg-gray-100 text-gray-500'}`}>Fill</button>
            <button onClick={() => setColorTab('border')}
              className={`flex-1 text-[11px] py-1 rounded font-bold ${colorTab === 'border' ? 'bg-gray-700 text-white' : 'bg-gray-100 text-gray-500'}`}>Border</button>
            <button onClick={() => setColorTab('text')}
              className={`flex-1 text-[11px] py-1 rounded font-bold ${colorTab === 'text' ? 'bg-gray-700 text-white' : 'bg-gray-100 text-gray-500'}`}>Text</button>
          </div>
          <div className="mb-2 max-h-28 overflow-y-auto pr-1">
            <div className="flex flex-wrap gap-1.5">
              {WIDGET_COLORS.map((c, i) => {
                const active = colorTab === 'bg'
                  ? widgetBg === c
                  : colorTab === 'border'
                    ? widgetBorder === c
                    : widgetText === c;
          ...
          <div className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Style</div>
      ...
      <div
        ...
        {widgetStyle === 'glass' && !skipAnimations && (
          <>
            ...
            <div
              ...
              style={{
                background: 'linear-gradient(135deg, rgba(255,255,255,0.85) 0%, rgba(255,255,255,0.12) 45%, rgba(255,255,255,0.5) 100%)',
            ...
            <div
              className="absolute inset-0 pointer-events-none"
              style={{
                background: 'radial-gradient(circle at 18% 12%, rgba(125,211,252,0.45), transparent 48%), radial-gradient(circle at 82% 88%, rgba(167,139,250,0.35), transparent 55%)',
                borderRadius: 'inherit',
        ...
        {widgetStyle === 'aurora' && !skipAnimations && (
          <>
            ...
            <div
              className="absolute inset-0 pointer-events-none"
              style={{
                background: 'linear-gradient(180deg, rgba(255,255,255,0.85) 0%, rgba(255,255,255,0.25) 50%, rgba(255,255,255,0.7) 100%)',
                borderRadius: 'inherit',
                opacity: 0.7,
              }}
            />
            <div
              className="absolute inset-0 pointer-events-none"
              style={{
                background: 'radial-gradient(circle at 20% 30%, rgba(56,189,248,0.45), transparent 50%), radial-gradient(circle at 80% 70%, rgba(167,139,250,0.4), transparent 55%), radial-gradient(circle at 50% 90%, rgba(34,197,94,0.25), transparent 50%)',
                borderRadius: 'inherit',
        ...
        {widgetStyle === 'gold' && (
          <div
            className="absolute inset-0 pointer-events-none"
            style={{
              background: 'linear-gradient(160deg, rgba(255,236,153,0.9) 0%, rgba(251,191,36,0.6) 35%, rgba(202,138,4,0.35) 100%)',
              borderRadius: 'inherit',
        ...
        {widgetStyle === 'wood' && (
          <div
            className="absolute inset-0 pointer-events-none"
            style={{
              background: 'linear-gradient(90deg, rgba(146,64,14,0.25) 0%, rgba(120,53,15,0.2) 25%, rgba(180,83,9,0.2) 50%, rgba(120,53,15,0.2) 75%, rgba(146,64,14,0.25) 100%)',
              borderRadius: 'inherit',
              opacity: 0.8,
            }}
          />
        ...
        {widgetStyle === 'neon' && !skipAnimations && (
          <>
            <div
              className="absolute inset-0 pointer-events-none"
              style={{
                background: 'repeating-linear-gradient(90deg, rgba(255,255,255,0.05) 0 18px, transparent 18px 36px)',
                borderRadius: 'inherit',
                mixBlendMode: 'screen',
                opacity: 0.4,
              }}
