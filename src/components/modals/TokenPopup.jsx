      ...
      >
        ...
        {goal.active ? (
          <div className="p-4">
            ...
            <div className="flex flex-wrap gap-1 justify-center mb-3">
              {Array.from({ length: goal.goal }).map((_, i) => (
                <div
                  key={i}
                  className={`w-7 h-7 rounded-full flex items-center justify-center text-sm border-2 ${i < goal.tokens ? 'bg-yellow-400 border-yellow-500 text-yellow-800' : 'bg-gray-100 border-gray-300 text-gray-400'}`}
                >
            ...
            <div className="text-center text-sm text-gray-600 mb-3">
              Reward: <span className="font-medium">{goal.reward}</span>
            </div>
            {isComplete && (
            ...
            <div className="flex gap-2 justify-center">
              ...
              {isComplete ? (
                <button
                  ...
                  className="px-4 py-2 rounded-lg bg-amber-100 text-amber-700 font-bold text-sm hover:bg-amber-200"
                >
                  ↺ Reset
                </button>
              ) : (
                <button
                  onClick={onAddToken}
                  className="px-4 py-2 rounded-lg font-bold text-sm bg-green-100 text-green-600 hover:bg-green-200"
                >
                  + Add Token
                </button>
              )}
          ...
          <div className="p-4 text-center text-gray-500 text-sm">
            No active goal set for this student.
          </div>
        )}
      </div>
    </div>
