      ...
      <div className="flex flex-wrap gap-1">
        {pageEmojis.map(em => (
          <button key={em} onClick={() => onSelect(em)}
            className={`w-7 h-7 rounded text-base hover:bg-gray-100 ${selected === em ? 'bg-purple-100 ring-1 ring-purple-400' : ''}`}>{em}</button>
        ))}
      </div>
      {totalPages > 1 && (
        <div className="flex items-center justify-end gap-1 mt-1 pt-1 border-t border-gray-100">
          <span className="text-[10px] text-gray-400 mr-auto">{page + 1}/{totalPages}</span>
          <button
